...
<!-- SIDEBAR -->
 <aside class="w-64 bg-gray-900 text-white flex flex-col hidden md:flex">
    ...
    <div class="px-4 border-t border-gray-800 text-sm text-gray-400">
    ...
    <header class="bg-white shadow-sm h-16 flex items-center justify-between px-6 z-10">
       ...
       <div>
            <form action="{{ route('logout') }}" method="POST">
                @csrf
                <button type="submit" class="bg-red-500 hover:bg-red-600 text-white text-sm font-semibold px-4 py-2 rounded-lg transition">Logout</button>
            </form>
        </div>
    </header>
